        ...
        */
        ...
        for ($i = 1; $i <= 250; $i++) {
            ...
            if (isset($this->_dataIpn['payment_gross_' . $i])) {
                ...
                $this->_logger->info('Masspay _statusCompleted dataset: id = ' . print_r($payout['id']) . ' - old status = '
                    . print_r($payout['status']));
                if (isset($payout) && $payout['status'] < $payoutTable::$PAYOUT_STATUS_COMPLETED) {
                    $payoutTable->update(array("status"                     => $payoutTable::$PAYOUT_STATUS_COMPLETED,
        ...
        */
        ...
        for ($i = 1; $i <= 250; $i++) {
            if (isset($this->_dataIpn['payment_gross_' . $i])) {
                ...
                $payoutTable->update(array("status"                     => $payoutTable::$PAYOUT_STATUS_DENIED,
                                           "timestamp_masspay_last_ipn" => new Zend_Db_Expr('Now()'),
                                           "last_paypal_ipn"            => $this->_dataRaw,
                                           "last_paypal_status"         => $status_x
                ), "id = " . $unique_id_x);
        ...
        */
        ...
        for ($i = 1; $i <= 250; $i++) {
            if (isset($this->_dataIpn['payment_gross_' . $i])) {
                ...
                //check if old status < 100
                $payout = $payoutTable->fetchRow("id = " . $unique_id_x);
                $this->_logger->info('Masspay _statusProcessed dataset: id = ' . print_r($payout['id']) . ' - old status = '
                    . print_r($payout['status']));

                if ($payout && $payout['status'] < $payoutTable::$PAYOUT_STATUS_PROCESSED) {
                    $payoutTable->update(array("status"                     => $payoutTable::$PAYOUT_STATUS_PROCESSED,
                                               "timestamp_masspay_last_ipn" => new Zend_Db_Expr('Now()'),
                                               "last_paypal_ipn"            => $this->_dataRaw,
                                               "last_paypal_status"         => $status_x
                    ), "id = " . $unique_id_x);
