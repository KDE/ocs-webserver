...
namespace Library\Payment\PayPal\SubscriptionPayment;
 ...
 * Class Ipn
 *
 * @package Library\Payment\PayPal\SubscriptionPayment
 */
abstract class Ipn extends Base
{
    ...
    public function processIpn($rawData)
    {
        ...
        try {
            ...
            $this->_logger->err($exc->getTraceAsString());
            throwException($exc);
        }
    ...
    {
        $this->_logger->info(' ' . __FUNCTION__ . ' IPN: ' . print_r($this->_ipnMessage,
                true) . ' Status: ' . $this->_ipnMessage->getStatus());
        switch ($this->_ipnMessage->getStatus()) {
            case 'COMPLETED':
                $this->_statusSubscriptionPaymentCompleted();
                break;
            case 'PENDING':
                $this->_logger->info(' ' . __FUNCTION__ . ' Status = ' . $this->_ipnMessage->getStatus() . ': Nothing to do...');
                break;
            /*case 'Denied':
                $this->_statusDenied();
                break;
            case 'Processed':
                ...
                break;*/ default:
            $this->_logger->info(' ' . __FUNCTION__ . ' Status not found: . IPN: ' . print_r($this->_ipnMessage,
                    true) . ' Status: ' . print_r($this->_ipnMessage->getStatus(), true));
            throw new Exception('Unknown status from PayPal: ' . print_r($this->_ipnMessage));
        }
