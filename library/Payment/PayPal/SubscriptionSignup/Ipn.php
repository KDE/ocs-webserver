...
namespace Library\Payment\PayPal\SubscriptionSignup;
 ...
 * Class Ipn
 *
 * @package Library\Payment\PayPal\SubscriptionSignup
 */
abstract class Ipn extends Base
{
    ...
    {
        $this->_logger->info(' ' . __FUNCTION__ . ' - Data: ' . $rawData);
        ...
        $this->_logger->info(' ' . __FUNCTION__ . ' - Orgin Verified');

        $this->_dataRaw = $rawData;

        $this->_dataIpn = $this->_parseRawMessage($rawData);
        $this->_logger->info(' ' . __FUNCTION__ . ' - _parseRawMessage: ' . $this->_dataIpn['txn_type']);
        ...
        $this->_logger->info(' ' . __FUNCTION__ . ' - Transaction Valid');
    ...
    public function verifyIpnOrigin($rawDataIpn)
    {
        ...
        if (!($res = curl_exec($ch))) {
            $this->_logger->err(" " . __FUNCTION__ . " Url: " . $url);
            $this->_logger->err(" " . __FUNCTION__ . " Params: " . $req);
