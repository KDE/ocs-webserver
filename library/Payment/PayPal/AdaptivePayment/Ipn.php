...
abstract class Ipn extends Base
{
    ...
    public function verifyIpnOrigin($rawDataIpn)
    {
        ...
        $this->_logger->info(" " . __FUNCTION__ . " Url: " . $url);
        $this->_logger->info(" " . __FUNCTION__ . " Params: " . $requestParams);
    ...
    protected function _processRequest($requestData, $url)
    {
        ...
        if (false === $response) {
            $this->_logger->err(__FUNCTION__ . "::Error while request PayPal Website.\n Server replay was: " . $http->getLastRawResponse()
                                                                                                                    ->getStatusCode() . ". " . $http->getLastRawResponse()
                                                                                                                                                    ->getContent() . "\n");
            $this->_logger->err(__FUNCTION__ . '::Last Request: ' . print_r($http->getLastRawRequest(), true));
        } else {
            $this->_logger->debug(__FUNCTION__ . '::Last Request: ' . print_r($http->getLastRawRequest(), true));
    ...
    protected function processPaymentStatus()
    {
        switch ($this->_dataIpn['status']) {
            ...
            case 'PENDING':
                ...
                break;
            default:
                throw new Exception('Unknown status from PayPal: ' . $this->_ipnMessage->getStatus());
        }
    }
    ...
    /**
     * Some transfers succeeded and some failed for a parallel payment or, for a delayed chained payment, secondary
    ...
    protected function processTransactionStatus()
    {
        switch (strtoupper($this->_ipnMessage->getTransactionStatus())) {
            ...
            case 'DENIED':
                ...
                break;
            default:
                throw new Exception('Unknown transaction status from PayPal: ' . $this->_ipnMessage->getTransactionStatus());
        }
    }
