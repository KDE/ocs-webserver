...
DELIMITER $$
CREATE DEFINER = CURRENT_USER TRIGGER `member_external_id_BEFORE_INSERT` BEFORE INSERT ON `member_external_id` FOR EACH ROW
  BEGIN
...
INSERT INTO member_external_id (external_id, member_id)
  SELECT SUBSTR(SHA(`member_id`), 1, 20) AS external_id, member_id
  FROM member
