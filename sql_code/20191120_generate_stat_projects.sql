CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `v_project_follower` AS 
select f.* from project_follower f 
inner join member m on f.member_id = m.member_id and m.is_deleted = 0 and m.is_active = 1
    ...
    (PRIMARY KEY `primary` (`project_id`), INDEX `idx_ppload` (`ppload_collection_id`), INDEX `idx_cat` (`project_category_id`),INDEX `idx_member` (`member_id`),INDEX `idx_source_url` (`source_url`(50)))
      ...
      AS
        SELECT
          ...
          (select count(1) from `v_project_follower` where `v_project_follower`.`project_id` = `project`.`project_id`) as count_follower
        FROM
          `project`
