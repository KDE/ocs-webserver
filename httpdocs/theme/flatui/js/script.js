...
var ImagePreview = {
    ...
    initProductPicture: function () {
        ...
        $(imageTarget).attr('src', $('#image_small').data('url'));
        $(imageTarget).show();
        $('button#add-product-picture').text('CHANGE LOGO');
    },
...
var AboutMeMyProjectsPagingNew = (function () {
    return {
        setup: function () {
            $(window).scroll(function () {
                ...
                if (distance < 300) {
                    if (!$('#user-aboutme').hasClass("active")) return;
    ...
    return {
        setup: function () {
            ...
            $('body').on('click', 'a.aboutmetab', function (event) {
                ...
                var url = $(this).attr('data-href');
                if ($(el).find('.product').length == 0) {    // only load once
    ...
    return {
        setup: function () {
            ...
            $('body').on('click', 'a.communitytab', function (event) {
                ...
                var el = $(this).attr('href');
                var url = $(this).attr('data-href');
                if ($(el).find('.list').find('.user').length == 0) {    // only load once
            ...
            $('body').on('click', 'button.topic-tags-btn', function (event) {
                ...
                if ($(this).text() == 'Done') {
                    ...
                    $.each(lis, function (index, value) {
                        newhtml = newhtml + '<a rel="nofollow" href="/find?search=' + value.title + '/t/' + value.title + '/f/tags" '
