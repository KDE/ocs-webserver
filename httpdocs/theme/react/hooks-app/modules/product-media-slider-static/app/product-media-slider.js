...
function ProductMediaSlider(){ 
  ...
  function toggleCinemaMode(){
    ...
    const windowScrollTo = $("#product-media-slider-container").offset().top;
    window.scrollTo({
        top: windowScrollTo - 40
        // behavior: 'smooth'
    });
  }
  ...
  // go next 
  function goNext(){
    let nextSlide = mySwiper.activeIndex + 1;
    if (nextSlide > gallery.length - 1) nextSlide = 0;
    mySwiper.slideTo(nextSlide)
  }

  // go prev
  function goPrev(){
    let prevSlide =  mySwiper.activeIndex  - 1;
    if (prevSlide < 0 ) prevSlide = gallery.length - 1;
    mySwiper.slideTo(prevSlide)
  }
...
function SlideItem(props){
  ...
  if (props.slide.type === "embed"){
    let iFrameElement = <iframe src={props.slide.url}></iframe>;
    if (props.slide.url.indexOf('<iframe') > -1){
      iFrameElement = <div className="w-iframe" dangerouslySetInnerHTML={{__html:props.slide.url}}></div>
    } else if (props.slide.url.indexOf('<!DOCTYPE') > -1){
      iFrameElement = <iframe srcDoc={props.slide.url}></iframe>;
    }
    slideContentDisplay = (
    ...
    (props.currentSlide,'currentSlide')
    (props.props.slideIndex,'slideIndex');
    slideContentDisplay = (
