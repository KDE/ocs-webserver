...
function BookReaderWrapper(props){
  ...
  if (loading === false){
    ...
    if (showPrevButton === true){
      prevButtonDisplay = (
        <span id="prev-button"><a onClick={() => goPrev()}>{"< previous"}</a></span>
      )
    }
    ...
    if (totalPages !== 0){
      bookNavigationMidDisplay = (
        <span>
          <input type="number" className="form-control" placeholder={currentPage} min="0" max={parseInt(totalPages)} onChange={(e) => onPageNumberInput(e.target.value)}/>
          {" / " + totalPages}
        </span>
      )
    }
