...
function ComicBookReader(props){
  ...
  function initComicReader(){
    ...
    if (bookBlockElement){
      $(document).ready(function() {
        window.comicSwiper = new Swiper('#comics-reader'+props.slideIndex , {
          speed: 400,
          initialSlide: 0,
          observer: true, 
          observeParents: true,
        ...
        if (window.comicSwiper) window.comicSwiper.update()
      });
    } else {
  ...
  if (loading) comicPages = <span id="ajax-loader"></span>
    ...
    const comicPages = pages.map((p,index) => (
      <div className="swiper-slide" key={index}>
        <img onClick={props.onCinemaModeClick} src={p}/>
      </div>      
    ))
  ...
  return (
    <div id={"comics-reader"+props.slideIndex} className={"comic-book-reader swiper-container " + viewMode}>
      {comicBookDisplay}
      <div className="nav-container">
        <nav>
          <a id="bb-nav-counter">{ ( currentPage + 1 ) + "/" + totalPages }</a>
          <a id="bb-nav-first" onClick={() => onComicReaderNavClick('first')}><i className="bi bi-chevron-bar-left"></i></a>
          <a id="bb-nav-prev" onClick={() => onComicReaderNavClick('prev')}><i className="bi bi-chevron-left"></i></a>
          <a id="bb-nav-next" onClick={() => onComicReaderNavClick('next')}><i className="bi bi-chevron-right"></i></a>
          <a id="bb-nav-last" onClick={() => onComicReaderNavClick('last')}><i className="bi bi-chevron-bar-right"></i></a>
        </nav>
