...
function MusicPlayer(props){
  ...
  function onUpdateCurrentTrackProgress(newTrackProgress){
    //if (newTrackProgress !== 0){
      setCurrentTrackProgress(newTrackProgress);
      const playerElement = document.getElementById("music-player-container").getElementsByTagName('audio');
      const newCurrentTrackTime = (playerElement[0].duration / 100) * newTrackProgress;
...
function MusicPlayerControlPanel(props){
  ...
  React.useEffect(() => {
    ...
    // console.log(props);
    const parent = document.getElementById('music-player-container');
    let rcSlider = parent.getElementsByClassName('rc-slider')[0];
    if (rcSlider) rcSlider.addEventListener('click', onProgressBarClick);
    let rcVolumeSlider = parent.getElementsByClassName('rc-slider rc-slider-vertical')[0];
    if (rcVolumeSlider) rcVolumeSlider.addEventListener('click', onVolumeBarClick);
  },[])

  function onProgressBarClick(params) {
    const parent = document.getElementById('music-player-container');
    const divWidth = parent.getElementsByClassName('rc-slider')[0].offsetWidth;
    const precentage = (params.offsetX / divWidth ) * 100;
    if (precentage !== props.currentTrackProgress){
      props.onUpdateCurrentTrackProgress(precentage);
    }
  }

  function onVolumeBarClick(params) {
    const parent = document.getElementById('music-player-container');
    const divHeight = parent.getElementsByClassName('rc-slider rc-slider-vertical')[0].offsetHeight;
    const precentage = ((divHeight - params.offsetY) / divHeight ) * 100;
    if (precentage !== props.currentTrackProgress) props.onChangeAudioVolume(precentage / 100);    
  }
