...
export function GenerateGalleryArray(product, filesJson, galleryPics){
    let galleryArray = [];
    let isComics = false;
    let noGallery = false, noLogo = false;

    let galleryPictures = galleryPics;
    if (!galleryPics) galleryPictures = window.galleryPicturesJson;

    if (galleryPictures){
        galleryPictures.forEach(function(gp){  galleryArray.push({url:GenerateGalleryPictureUrl(gp),type:'image'}) });
        noGallery = true;
    }
    else {
        galleryArray = [{url:GenerateGalleryPictureUrl(product.image_small),type:'image'}];
    ...
    if (filesJson === null) filesJson = window.filesJson;
    if (filesJson) {
        filesJson.forEach(function(f,index){
            if (f.active === "1"){
                ...
                if (addFileToGallery === true){
                    ...
                    if (f.type.indexOf('audio') > -1 || type === 'audio'){
                        ...
                        let coverUrl;
                        if (galleryPictures) coverUrl = galleryPictures[0];
                        else { coverUrl = GenerateGalleryPictureUrl(product.image_small) }
                        gItem.cover = coverUrl;
...
function GenerateGalleryPictureUrl(url){
    let fullUrl = url;
    if (fullUrl.indexOf('https://') === -1){
        const urlSuffix = window.location.hostname.endsWith('com') ? 'com' : 'cc';
        fullUrl = "https://cdn.pling."+urlSuffix+"/img/"+url;
    }
    return fullUrl;
}
