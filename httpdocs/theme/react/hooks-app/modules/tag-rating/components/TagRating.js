...
const TagRating = () => {
    ...
    const loadTagRatings=()=>{
        ...
        const url = '/p/' + window.product.project_id + '/loadtagrating?gid=' + window.product.project_category_id;
        Axios.get(url)
            .then(result => {
    ...
    const handleSubmitVote= event =>{        
        ...
        const url = '/p/' + project_id + '/votetagrating';
    ...
    const loadProjectComments = event =>{        
        const url = window.config.baseUrlStore + '/p/' + window.product.project_id+'/loadcomments';
        jQuery.ajax({            
            url: url,
