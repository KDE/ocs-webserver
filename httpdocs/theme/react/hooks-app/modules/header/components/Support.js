        ...
        let  content = this.props.section.supporters.sort((a, b) => Number(a.active_months) < Number(b.active_months)).map((mg,i) => (
          <div className="section" key={i}>
            <div className="section-name">
              <a href={"/u/"+mg.username}><img src={mg.profile_image_url}></img></a>
              <span><a href={"/u/"+mg.username}>{mg.username}</a></span>
            </div>
            <div className="section-value"> {mg.period}</div>
          </div>
        ))
      ...
      <div className={"pling-section-header "+this.props.headerStyle}>
          <div className="header-body">
            <div ref={node => this.node = node} className={'supporter-container '+this.state.dropdownClass }>
              <a className="header-supporters">
                {this.props.section ? this.props.section.name : ''} has {s} active Supporters &#8964; 
              </a>
              {supporters}
            </div>
          </div>
