export function getNumberOfItemsPerRow(browseListType,isMobile,containerWidth){
    ...
    let itemsPerRow;
    if (isMobile){ 
        if (browseListType === "skills" || browseListType === "standard") itemsPerRow = 1;
        else itemsPerRow = 2;
    }
    else {
        if (browseListType === "music" || browseListType === "music-test") itemsPerRow = getAdujustItemsPerRow(7,containerWidth,146 + 14)
        ...
        else if (browseListType === "picture") itemsPerRow = getAdujustItemsPerRow(3,containerWidth,250 + 14)
        else if (browseListType === "favorites" ) itemsPerRow = getAdujustItemsPerRow(1,containerWidth,containerWidth - 1);
        else if (browseListType === "standard") itemsPerRow = getAdujustItemsPerRow(1,containerWidth,containerWidth - 1);
...
export function getItemWidth(browseListType,containerWidth,itemsInRow){
    ...
    else if (browseListType === "videos" || browseListType === "picture") itemWidth = (containerWidth / itemsInRow ) - 2;
    else if (browseListType === "comics") itemWidth = 160 + 14;
    else if (browseListType === "phone-pictures") itemWidth = itemWidth = (containerWidth / itemsInRow ) - 2;
    else if (browseListType === "books") itemWidth = 180 + 14;
    else if (browseListType === "standard") itemWidth = containerWidth;
    return itemWidth;
}
