...
const SearchForm = () => {
  ...
  const onSearchFormSubmit = e => {
    ...
    if (!selected) {
      ...
      {
        window.location.href = state.searchbaseurl+'?search=' + value;
      }else{
        window.location.href = state.searchbaseurl;
      }
      ...
      if (selected.type == 'project') {
        ...
        window.location.href = '/p/' + selected.project_id;
        if(state.target.target=='pling'|| state.target.target=='opendesktop')
        {
          window.location.href = '/p/' + selected.project_id;
        }else{
          window.location.href = state.baseUrlStore + '/p/' + selected.project_id;
  ...
  const onSuggestionSelected = (event, { suggestion, suggestionValue, suggestionIndex, sectionIndex, method }) => {    
    if (suggestion.type == 'project') {
      ...
      //window.location.href = '/p/' + suggestion.project_id;
      if(state.target.target=='pling'|| state.target.target=='opendesktop')
        {
          window.location.href = '/p/' + suggestion.project_id;
        }else{
