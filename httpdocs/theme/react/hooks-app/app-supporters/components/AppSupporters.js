...
const AppSupporters = () => {
  ...
  const showDetail = sc =>{
      setShowContent(sc);      
      if(sc=='overview'){ loadData(section);}
      if(sc=='recentplings'){ loadrecentplings(section);}    
  }
  ...
  if (sections){
    ...
    if(showContent=='supporters')
    {
      detailContent = <Supporters baseUrlStore={state.baseurlStore} supporters = {supporters}/>
        ...
        detailContent = <div>
                        <RecentPlinged  baseUrlStore={state.baseurlStore} products={recentplings} ></RecentPlinged>
    ...
    sectiondetail = <div className="pling-section-detail">
                    { section &&
                      <div className="pling-section-detail-left">
                        <h2 className={showContent=='overview'?'focused':''}><a onClick={()=>showDetail('overview')}>Content</a></h2>
