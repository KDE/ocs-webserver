 ...
 **/
class Default_Model_DbTable_VConfigStoreCategory extends Local_Model_Table
{

    protected $_keyColumnsForRow = array('store_category_id');
    protected $_key = 'store_category_id';
    protected $_name = "v_config_store_category";
    ...
    public function fetchAllCategoriesForStore($storeId)
    {
        ...
        $sql = "
                ...
                FROM v_category AS pc, v_category AS pc2
                WHERE pc.v_category_id IN (SELECT DISTINCT csc.v_category_id
                                                 FROM v_config_store_category AS csc
                                                   JOIN v_category AS pc ON csc.v_category_id = pc.v_category_id
                                                 WHERE csc.store_id = :storeId)
                ORDER BY pc2.v_category_id;
                ";
    ...
    {
        $sql = "
            SELECT csc.v_category_id 
            FROM v_config_store_category AS csc
            JOIN v_category AS pc ON pc.v_category_id = csc.v_category_id
        ...
        $values = array_map(function($row) { return $row['v_category_id']; }, $results);
        return $values;
    }
