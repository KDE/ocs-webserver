...
class Default_Model_StatDownload
{
    ...
    public function getUserDownloads($member_id)
    {
        $sql = "
                SELECT 
                    member_dl_plings.*,
                    (select count(1) as sum_plings from project_plings pp where pp.project_id = member_dl_plings.project_id and pp.is_deleted = 0 and is_active = 1 group by pp.project_id) as sum_plings,
                    project.title,
                    project.image_small,
