...
use Application\Model\Service\Interfaces\ActivityLogServiceInterface;
use Exception;
use Laminas\Db\Adapter\Adapter;
use Laminas\Db\TableGateway\Feature\GlobalAdapterFeature;

class ActivityLogService extends BaseService implements ActivityLogServiceInterface
{
    ...
    const PROJECT_COMMENT_VOTE_ADDED = 44;
    const PROJECT_COMMENT_VOTE_REMOVED = 45;
    const PROJECT_COMMENT_VOTE_UPDATED = 46;
    const PROJECT_COMMENT_VOTE_DELETED = 47;
    ...
    const PROJECT_VOTE_ADDED = 62;
    const PROJECT_VOTE_REMOVED = 63;
    const PROJECT_VOTE_UPDATED = 64;
    ...
    public static function logActivity($objectId, $projectId, $userId, $activity_type_id, $data = array())
    {
        ...
        try {
            ...
            $query = $adapter->query($sql, Adapter::QUERY_MODE_PREPARE);
    ...
    public function writeActivityLog($objectId, $userId, $activity_type_id, $data)
    {
        ...
        //is it an item?
        if (isset($data['type_id']) && $data['type_id'] == ProjectService::PROJECT_TYPE_UPDATE) {
            $projectId = $data['pid'];
        }
        $object_text = (strlen($data['description']) < 150) ? $data['description'] : mb_substr(
                                                                                         $data['description'], 0, 145, 'UTF-8'
        ...
        try {
            $this->activityLogRepository->insertOrUpdate($newLog);
        } catch (Exception $e) {
            $this->log->err(__METHOD__ . ' - ERROR write activity log - ' . print_r($e, true));
        }
    }
