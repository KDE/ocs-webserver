...
class OAuth
{
    ...
    public function __construct(
        Config $config,
        OAuthServer $httpServer,
        MemberInterface $modelMember
    ) {
        $this->messages = array();
        if (isset($config)) {
            $this->config = $config;
        ...
        // new Default_Model_Ocs_HttpTransport_OAuthServer($this->config)
    ...
    {
        $member = $this->modelMember->fetchMemberData($member_id, false);

        if (empty($member)) {
            throw new Exception('member with id ' . $member_id . ' could not found.');
        }
    ...
    public function updateUser($member)
    {
        ...
        return $this->createUserFromArray($member, $force = true);
    }
