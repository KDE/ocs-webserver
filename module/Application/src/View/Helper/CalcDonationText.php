...
class CalcDonationText extends AbstractHelper
{
    ...
    {
        return $this->calcDonationText($tier);
    }
    ...
    public function calcDonationText($tier)
    {
        ...
        $t = $total * self::tax;
        $g = $total + $t;
        $r = '($' . $tier . ' + $' . number_format($paypal, 2) . ' paypal + $' . number_format($t, 2) . ' tax = $' . number_format($g, 2) . ' monthly)';
